const template = `<form-item ref="spreadRef" class="spread-form-item" :class="{ open: open, disabled: disabled }" :label="label" :desc="desc" @click="spreadItem">
	<span
		...
		class="spread-show-text">{{ spreadShowText }}</span>
	<template v-else>
...
export default defineComponent( {
	name: "SpreadFormItem", template, components: {
		FormItem, Tags
	}, emits: [ "update:modelValue", "change", "open", "close" ], props: {
		modelValue: {
			default: ""
		},
		/* 当前展开的组件id */
		...
		type: {
			default: "normal", validator( value ) {
		...
		/* 校验错误提示文字 */
		verifyMsg: {
			type: String, default: "请检查填写内容"
		}, disabled: {
			type: Boolean, default: false
		}
	}, setup( props, { emit } ) {
		const state = reactive( {
			open: false, showErrMsg: false, formValue: ""
		} );
		...
		const spreadShowText = computed( () => {
			return showBtnStyle.value ? "点击设置" : state.formValue instanceof Array ? state.formValue.join( "、" ) : state.formValue
		} );
		...
		return {
			...toRefs( state ), inputRef, spreadRef, showBtnStyle, spreadShowText, spreadItem, packUpItem, saveValue
		}
	}
} )

...
export async function getHeaders( userId: string, accountId ): Promise<HEADERS> {
	//获取用户信息填充header
	const dbKey = `extr-wave-yys-sign-${ userId }-${ accountId }`;
	let headers: HEADERS = {
		'x-rpc-combo_token': "",
		'x-rpc-client_type': 2,  //默认安卓
		'x-rpc-app_version': "3.3.0", //当前云原神版本

...
export async function main(
	...
	const SUBSCRIBE = <Order>command.getSingle( "extr-wave-yysign-enable", AuthLevel.User );
	const YCONFIRM = <Order>command.getSingle( "extr-wave-yysign-confirm", AuthLevel.User );
	
	if ( SUBSCRIBE.getHeaders().includes( header ) ) {
		const msg: string = await subscribe( userID, sendMessage, AuthLevel.User, YCONFIRM );
		await sendMessage( msg );
	} else if ( YCONFIRM.getHeaders().includes( header ) ) {
		const msg: string = await confirm( userID, data, AuthLevel.User, SUBSCRIBE );
		await sendMessage( msg );
	}
}

...
export async function main( { sendMessage, messageData }: InputParameter ): Promise<void> {
	...
	if ( result.definite ) {
		...
		if ( typeof check === "boolean" ) {
			if ( check ) {
				message = `https://adachi-bot.oss-cn-beijing.aliyuncs.com/Version2/guide/${ result.info }.png`;
				await sendMessage( { image: encodeURI( message ).toString() } );
				return;
			}
			message = `未查询到关于「${ result.info }」的攻略图，请等待西风驿站上传后再次查询，或前往 Extrwave/Adachi-GBOT 进行反馈`;
		} else {
			message = "获取西风攻略图出错：" + check;
		}
	} else if ( result.info === "" ) {
		message = `暂无关于「${ name }」的信息，若确认名称输入无误，联系开发者进行反馈`;
	} else {

...
export default class Interval {
    ...
    public check( userID: string, groupID: string ): boolean {
    ...
    public get( userID: string, groupID: string ): number {
        return groupID === "-1" ? this.p.get(userID) : this.g.get(groupID);
    }

    public async set( id: string, type: string, time: number ): Promise<void> {
